...
class HF_tools():
    ...
    def __init__(self):
        self.K = 2 #NUMBER OF BASIS FUNCTIONS
        self.N = 2 #NUMBER OF ELECTRONS
        self.E = np.zeros(self.K)
        self.r_bas = [0.0   ,1.5517] # CENTERS OF BASIS SETS
        self.a     = [0.4166,0.7739]
        self.r_nuc = [0.0   ,1.5517] # CENTER OF NUCLEOUS
        self.Z     = [1.0   , 2.0] # CHARGE OF NUCLEOUS
    ...
    def get_total_E(self):
        ...
        for i in range(self.K):
            for j in range(self.K):
                total_E += 0.5*self.P[i,j]*(self.T[i,j] +self.A[i,j] + self.F[i,j])
        return total_E
